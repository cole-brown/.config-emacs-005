;;; namespaced/dlv/path.el --- Path helpers for DLV. -*- lexical-binding: t; -*-
;;
...
(defun _:dlv:path:expand (path &optional as-dir)
...
Else return path as type (dir/file) provided."
  (when (or (null path)
            (not (stringp path)))
    (error "_:dlv:path:expand: PATH must be a string! Got: %S %S"
           (type-of path)
           path))
  ...
  (let ((path/abs (expand-file-name
    ...
    (if (string-prefix-p "~" path)
      ...
      path/abs)))
;; (_:dlv:path:expand "~/foo/../bar")
;; (_:dlv:path:expand "~/foo/../bar" :dir)
;; Windows:
;;   (_:dlv:path:expand "d:/home/work/foo/../bar")
;; Error:
;;   (_:dlv:path:expand nil)

(defun _:dlv:dir:normalize (dir &optional preserve-tilde)
  "Normalize DIR into an absolute path with '/' separators and ending in a '/'.

Example:
  (_:dlv:dir:normalize \"~/foo/bar\")
    -> \"/home/work/foo/bar/\"
    NOTE: Loses the leading \"~\" unless PRESERVE-TILDE is non-nil!

  (_:dlv:dir:normalize \"~/foo/bar\" t)
    -> \"~/foo/bar/\"

  (_:dlv:dir:normalize \"~/foo/bar/\" t)
    -> \"~/foo/bar/\"

  (_:dlv:dir:normalize \"D:\\foo\\bar\")
    -> \"d:/foo/bar/\"

  (_:dlv:dir:normalize \"D:\\home\\work\\foo\\bar\")
    -> \"d:/home/work/foo/bar/\""
  ;; Finish by ensuring we have a trailing '/'.
  (file-name-as-directory
   ;; Do we want to preserve "~"?
   (if preserve-tilde
       (_:dlv:path:expand dir)
...
;; (_:dlv:dir:normalize "D:\\foo\\bar")
;; (_:dlv:dir:normalize "~/foo/bar")
;; (_:dlv:dir:normalize "~/foo/bar" :preserve)


(defun _:dlv:path:multiplex (path &optional as-dir)
...
Example:
  (_:dlv:path:multiplex \"~/foo/bar\")
    -> '((\"~/\" \"/home/jeff/\") . \"foo/bar\")

  (_:dlv:path:multiplex \"/home/jeff\")
    -> '((\"~/\" \"/home/jeff/\") . \"\")

  (_:dlv:path:multiplex \"/some/path/foo/bar\")
    -> '((nil) . \"/some/path/foo/bar\")"
  (let ((func/name '_:dlv:path:multiplex))
    (_:dlv:debug func/name
                 "args: PATH:%S AS-DIR:%S"
                 path
                 as-dir)
    ...
    (let* ((path/home.abbrev "~")
           ...
           (path/abs (_:dlv:path:expand path as-dir)))
      ...
      (let ((return
        ...
        return))))
;; (_:dlv:path:multiplex "~/foo/bar")
;; (_:dlv:path:multiplex "/etc/foo/bar")
;; (_:dlv:path:multiplex "d:/home/work")
;; (_:dlv:path:multiplex "d:/some/path/foo/bar")

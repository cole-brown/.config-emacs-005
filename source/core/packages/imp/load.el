...
Returns a plist:
  ...
  (let ((keys:valid '(:path :filename :feature :optional :error :skip))
      ...
      (let ((key   (car plist))
        ...
        (cond ((eq key :path)
              ...
              ((eq key :feature)
               ...
               (setq in:feature (imp--list-flatten value)))
              ((eq key :error)
               (setq in:error value))
              ((eq key :skip)
               (setq in:skip value))
    ...
    (let ((path (or in:path
                    (imp--path-root-dir (nth 0 out:feature) :no-error)
                    path:current-dir)))
      (unless (stringp path)
        (imp--error caller
...
Return nil for failure, non-nil for success."
  (let ((macro:path:current-dir (imp/path-current-dir)))
    `(let* ((macro:func/name "imp/load")
            (macro:parsed (imp--load-parse macro:func/name
            ...
            (macro:name:load   (imp--path-filename macro:path:load)) ;; Filename sans extension.
